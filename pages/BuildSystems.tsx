...
import { Github, Server, Terminal, Search, Rocket } from 'lucide-react';
...
export const BuildSystems: React.FC<BuildSystemsProps> = ({ 
    onLearn, 
    favorites = [], 
    toggleFavorite,
    externalSearchQuery = '',
  ...
  const displayTools = useMemo(() => {
    if (externalResearchResult) {
        return buildToolsData.filter(t => externalResearchResult.toolIds.includes(t.id));
    }
    if (externalSearchQuery) {
        const lower = externalSearchQuery.toLowerCase();
        return buildToolsData.filter(t => 
            t.name.toLowerCase().includes(lower) || 
            t.description.toLowerCase().includes(lower) ||
            t.tags.some(tag => tag.toLowerCase().includes(lower))
        );
    }
    return buildToolsData;
        ...
        {externalResearchResult && (
            <div className="mb-12 animate-fade-in mt-8">
                <div className="p-6 rounded-2xl bg-gradient-to-br from-emerald-900/40 to-cyan-900/40 border border-emerald-500/30 flex gap-4 items-start shadow-xl">
                    <div className="p-3 bg-emerald-500/20 rounded-full shrink-0"><Terminal className="text-emerald-300" size={24} /></div>
                    <div>
        ...
        {hasTools && (
            <div className="animate-fade-in">
                ...
                {freemiumTools.length > 0 && (
                    <div className="mb-20">
                        {/* Managed & Cloud - Gradient Header */}
                        <div className="flex items-center gap-2 mb-8">
                            <div className="bg-gradient-to-r from-blue-400 to-cyan-500 bg-clip-text text-transparent">
                                <Rocket size={24} className="text-blue-400 inline mr-3 align-middle" />
                                <h3 className="text-2xl font-bold tracking-wider uppercase inline align-middle">Managed & Cloud</h3>
                            </div>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                {paidTools.length > 0 && (
                    <div className="mb-20">
                        {/* Enterprise Infrastructure - Gradient Header */}
                        <div className="flex items-center gap-2 mb-8">
                            <div className="bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent">
                                <Server size={24} className="text-purple-400 inline mr-3 align-middle" />
                                <h3 className="text-2xl font-bold tracking-wider uppercase inline align-middle">Enterprise Infrastructure</h3>
                            </div>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
